...
def populate_db():
    ...
    menuItem3 = MenuItem(name="Spicy Tuna Roll", description="",
                         price="$4.99", course="Appetizer", restaurant=restaurant2)
    session.add(menuItem3)
    session.commit()

    menuItem4 = MenuItem(name="Nepali Momo", description="",
                         price="$8.99", course="Appetizer", restaurant=restaurant2)
    session.add(menuItem4)
    session.commit()
    ...
    menuItem6 = MenuItem(name="Ramen", description="",
    ...
    menuItem1 = MenuItem(name="Tres Leches Cake", description="Rich, luscious sponge cake soaked in sweet milk and topped with vanilla bean whipped cream and strawberries.",
                         price="$4.99", course="Dessert", restaurant=restaurant1)
    session.add(menuItem1)
    session.commit()
    ...
    menuItem3 = MenuItem(name="Honey Boba Shaved Snow",
                         description="Milk snow layered with honey boba, jasmine tea jelly, grass jelly, caramel, cream, and freshly made mochi", price="$4.99", course="Beverage", restaurant=restaurant1)
    session.add(menuItem3)
    session.commit()

    menuItem4 = MenuItem(name="Cauliflower Manchurian",
                         description="Golden fried cauliflower florets in a midly spiced soya,garlic sauce cooked with fresh cilantro, celery, chilies,ginger & green onions", price="$3.49", course="Appetizer", restaurant=restaurant1)
    session.add(menuItem4)
    session.commit()
    ...
    menuItem5 = MenuItem(name="Aloo Gobi Burrito", description="Vegan goodness. Burrito filled with rice, garbanzo beans, curry sauce, potatoes (aloo), fried cauliflower (gobi) and chutney. Nom Nom",
                         price="$9.99", course="Entree", restaurant=restaurant1)
    session.add(menuItem5)
    session.commit()
    ...
    menuItem4 = MenuItem(name="Choc Full O\' Mint (Smitten\'s Fresh Mint Chip ice cream)",
                         description="", price="$4.49", course="Dessert", restaurant=restaurant1)
    session.add(menuItem4)
    session.commit()
    ...
    menuItem1 = MenuItem(name="Lamb Curry", description="Slow cook that thang in a pool of tomatoes, onions and alllll those tasty Indian spices. Mmmm.",
                         price="$12.99", course="Entree", restaurant=restaurant1)
    session.add(menuItem1)
    session.commit()
    ...
    menuItem4 = MenuItem(name="Nigiri Sampler",
                         description="", price="$5.99", course="Appetizer", restaurant=restaurant1)
    session.add(menuItem4)
    session.commit()
    ...
    menuItem3 = MenuItem(name="Morels on toast (seasonal)",
                         description="Wild morel mushrooms fried in butter, served on herbed toast slices", price="$5.49", course="Appetizer", restaurant=restaurant1)
    session.add(menuItem3)
    session.commit()
    ...
    menuItem4 = MenuItem(name="Tandoori Chicken", description="Chicken marinated in yoghurt and seasoned with a spicy mix(chilli, tamarind among others) and slow cooked in a cylindrical clay or metal oven which gets its heat from burning charcoal.",
                         price="$8.99", course="Appetizer", restaurant=restaurant1)
    session.add(menuItem4)
    session.commit()
    ...
    menuItem1 = MenuItem(name="Super Burrito Al Pastor",
                         description="Marinated Pork, Rice, Beans, Avocado, Cilantro, Salsa, Tortilla", price="$11.99", course="Entree", restaurant=restaurant1)
    session.add(menuItem1)
    session.commit()
